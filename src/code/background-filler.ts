export default class BackgroundFiller {
    drawExplosion(
        ...
        explosionShadowColor: string,
        explosionFillColor: string,
        explosionStrokeColor: string,
    ) {
        ...
        ctx.save();
        ctx.translate(-22, 22);
        ctx.fillStyle = explosionShadowColor;
        ctx.fill(boomPath);
        ctx.restore();

        //Draw explosion
        ctx.fillStyle = explosionFillColor;
        ctx.strokeStyle = explosionStrokeColor;
        ctx.lineWidth = 7;
        ctx.fill(boomPath);
        ctx.stroke(boomPath);
    }

    drawColorPallet(ctx: CanvasRenderingContext2D, colors: string[][]) {
        let positionX = 0;
        let positionY = 0;
    ...
    drawCirclePattern(ctx: CanvasRenderingContext2D, radius: number, color: string, countInRow: number, countInColumn: number) {
        let positionY = 0;
